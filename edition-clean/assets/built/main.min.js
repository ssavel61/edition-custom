...
function initInfiniteScroll() {
    ...
    function fetchNextPage() {
        // Look for the hidden link in the pagination block
        var nextLink = document.querySelector('.older-posts');
        if (!nextLink) return;
        ...
        xhr.onload = function () {
            if (xhr.status >= 200 && xhr.status < 400) {
                ...
                var nextLinkBody = xhr.response.querySelector('.older-posts');

                newNodes.forEach(function (node) {
                    feed.appendChild(document.importNode(node, true));
                });

                // Update the hidden link for the next round
                var currentNextLink = document.querySelector('.older-posts');
                if (nextLinkBody && currentNextLink) {
                    currentNextLink.setAttribute('href', nextLinkBody.getAttribute('href'));
                } else {
